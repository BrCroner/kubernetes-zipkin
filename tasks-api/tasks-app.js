...
app.get('/tasks', async (req, res) => {
    try {
        ...
        fs.readFile(filePath, (err, data) => {
            if (err) {
                console.log(err);
                return res.status(500).json({ message: 'Carregamento das terefas falhou.' });
            }
        ...
        console.log(err);
        return res.status(401).json({ message: err.message || 'Falha ao carregar tarefas.' });
    }
});
...
app.post('/tasks', async (req, res) => {
    try {
        ...
        fs.appendFile(filePath, jsonTask + 'TASK_SPLIT', (err) => {
            ...
            res.status(201).json({ message: 'Tarefa armazenada.', createdTask: task });
        });
    } catch (err) {
        return res.status(401).json({ message: 'Não foi possível verificar token.' });
    }
});
